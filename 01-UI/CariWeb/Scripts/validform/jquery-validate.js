...
; (function (defaults, $, window, undefined) {
    ...
    var
		...
		validateField = function (event, options) {
		    ...
		    var patternName = fieldPattern;
		    if (settings.patterns[fieldPattern]) {
		        fieldPattern = settings.patterns[fieldPattern];
		    }
		    ...
		    if (field.is(type[0])) {
		        ...
		        if (fieldPattern.test(fieldValue)) {
		            ...
		            if (fieldRequired && field.data('required') != 'false') {
		    ...
		    //扩展代码
		    if (!status.required) {
		        if (field.attr('type') == 'checkbox' || field.attr('type') == 'radio' || field.is('select')) {
		            log = fieldDescription.required || settings.messages.requiredSelect;
		        } else {
		            log = fieldDescription.required || settings.messages.required;
		        }
		    } else if (!status.pattern) {
		        var msg = settings.messages[patternName] || '格式不正确!';
		        log = fieldDescription.pattern || msg;
		    ...
		    if (status.required && status.pattern && status.conditional) {
		        ...
		        if (field.hasClass('ng-invalid')) {
		            field.removeClass('ng-invalid').addClass('ng-valid');
		        }

		        if (validTarget.obj == field.get(0) && validTarget.show) {
		            field.trigger('tip-hide');
		            validTarget.show = false;
		        }
		    } else {
		        field.removeClass('ng-valid').addClass('ng-invalid');
		        if (log) {
		            validTarget.index++;
		            if (validTarget.index == 1) {
		                validTarget.show = false;
		                field.focus();
		            }

		            if (validTarget.show == false) {
		                field.miniTip({
		                    content: log,
		                    anchor: 's',
		                    stemOff: 0
		                }).trigger('tip-show');
		                validTarget.show = true;
		                validTarget.obj = field.get(0);
		            }
		        }
            ...
            return $(this).validateDestroy().each(function () {
                ...
                if (form.is('form')) {
                    ...
                    fields.each(function (i, field) {
                        var self = $(field);
                        if (self.data('required') != undefined) {
                            self.addClass('ng-invalid');
                        } else {
                            self.removeClass('ng-invalid');
                        }
                    })
    ...
    var settings = {
        messages: {
            ...
            maxlength: "请输入一个 长度最多是 {0} 的字符",
            minlength: "请输入一个 长度最少是 {0} 的字符",
            rangelength: "请输入 一个长度介于 {0} 和 {1} 之间的字符",
